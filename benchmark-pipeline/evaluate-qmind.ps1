...
if (-not [string]::IsNullOrWhiteSpace($Scenario)) {
    ...
    if ($caseId -notmatch "^OB-\d{3}$") {
        throw "Scenario must look like OB-001 through OB-006. Found: $Scenario"
    }
    ...
    if ([string]::IsNullOrWhiteSpace($Selection)) {
        throw "Selection is required when Scenario is not provided. Prefer -Scenario OB-001 through OB-006 for benchmark-case evaluation."
    }
    if ([string]::IsNullOrWhiteSpace($Output)) {
