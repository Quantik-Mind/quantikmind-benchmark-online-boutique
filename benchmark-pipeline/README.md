...
| OB-007 Runtime Observability Gap | Runtime Aware | `src/adservice/src/main/java/hipstershop/AdService.java` | Harmless adservice commit coincides with independently injected recommendationservice runtime pressure; no adservice tests exist in the library so the guardrail has no effect | Recommendation-section test selected from Prometheus-visible recommendationservice latency evidence; History + Code Change follows the adservice commit and misses the recommendation oracle |

For OB-005, History + Code Change receives only currency data changed-file context; it does not receive runtime metrics or oracle detecting tests. QMind detection must come from runtime observability showing elevated currencyservice and frontend error rates. For OB-006, the changed file points to productcatalogservice and QMind detection must come from combined productcatalogservice degradation and frontend homepage/product-grid impact. For OB-007, the declared changed file is a harmless adservice source file (`src/adservice/src/main/java/hipstershop/AdService.java`); runtime pressure is injected independently on recommendationservice, and QMind detection must come from Prometheus-visible recommendationservice latency evidence rather than any commit or guardrail signal.
