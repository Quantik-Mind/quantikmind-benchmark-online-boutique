...
The benchmark now models six independent CI/CD validation cases: OB-001 through OB-004 are code-change control cases, OB-005 is the runtime-aware case, and OB-006 is the combined-signal case. Each selector runs from allowed pre-execution inputs first, and the defect oracle is applied only afterward for scoring.
...
Read the final comparison as recall first, execution reduction second, with the category breakdown always shown. It is not a "lowest test count wins" benchmark: History + Code Change reaches 5/6 recall with 15.0 average tests and 70.6% execution reduction, while Quantik Mind reaches 6/6 recall with 15.8 average tests and 69.0% execution reduction. Quantik Mind spends 0.8 more tests on average to recover the runtime-aware case that History + Code Change misses.
...
The benchmark demonstrates value when static history/code-change signals alone are insufficient for the defined runtime-aware and combined-signal cases. The committed OB-005 and OB-006 QMind artifacts currently select the same frontend/homepage risk cluster and report identical business metrics, so OB-006 should not be presented as independent proof of an additional runtime-aware win until distinct live runtime evidence is captured. The benchmark does not demonstrate universal superiority across all applications, all defect classes, or all traffic/observability configurations. It also does not replace product onboarding documentation, production deployment guidance, or a full Online Boutique operations runbook.
...
Under the defined Online Boutique scenarios, Quantik Mind executed 15.8 tests on average out of 51, reduced execution volume by 69.0%, detected 6/6 benchmark defects, and captured 77.78% average critical observed risk. This demonstrates the value of dynamic runtime risk signals in these benchmark cases without claiming universal superiority.
...
- `benchmark-results/qmind-selections/qmind-selection-ob-001.json` through `qmind-selection-ob-006.json`: committed per-case QMind selections used by the final comparison.
...
- `run-qmind-subset.ps1`: runs a changed-files subset and normalizes the output to canonical test IDs. `generate-final-comparison.ps1` calls it automatically for `-BenchmarkCase OB-001` through `OB-006` in normal mode.
