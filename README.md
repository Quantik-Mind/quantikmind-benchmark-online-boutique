...
| OB-007 Recommendation Runtime Behavioral Degradation | Runtime Aware | Recommendation service (harmless logger change declared; runtime patch applied separately) | ListRecommendations delay causes recommendation section to disappear from product detail pages | Recommendation-section check on product detail pages, informed by Prometheus-visible recommendationservice latency evidence |

OB-005 and OB-007 validate the value of runtime observability: a code-change-only selector sees only the declared changed file and does not receive runtime metrics. OB-005 connects elevated currencyservice and frontend error signals to user-visible homepage checks. OB-007 connects Prometheus-visible recommendationservice latency evidence to the recommendation-section check on product detail pages. OB-006 validates combined-signal selection because the code change points to productcatalogservice, while runtime observability shows the direct user-visible impact through frontend homepage/product-grid behavior.
